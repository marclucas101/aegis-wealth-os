-- Verification for migration 202606290012 (Phase 08 Relationship moments feature control).
SELECT
  'phase08.feature.verify' AS check_id,
  CASE
  ...
  END AS crm_v2_relationship_moments_key,
  CASE
  ...
  END AS crm_v2_relationship_moments_enabled,
  CASE
    WHEN to_regclass('public.platform_feature_controls') IS NULL THEN 'platform_feature_controls_absent'
    ELSE COALESCE(
      (xpath('//row/feature_key/text()',
  ...
  END AS crm_v2_client_profile_key;
