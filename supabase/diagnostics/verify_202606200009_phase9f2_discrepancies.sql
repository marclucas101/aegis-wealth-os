-- Read-only discrepancy report for 202606200009_phase9f2_lifecycle_notifications.sql
-- Filtered view of the same resolved inventory as verify_202606200009_phase9f2_lifecycle_notifications.sql.
  ...
  SELECT
    ...
    CASE
      WHEN r.state = 'absent' AND r.check_kind = 'column' THEN
        'Phase 9F.2 column missing on client_notifications; migration 202606200009 may not be applied.'
      WHEN r.state = 'absent' AND r.check_kind = 'column_attr' THEN
        'Column attribute mismatch or column absent; compare migration DDL and live information_schema.'
      WHEN r.state = 'absent' AND r.check_kind IN ('index','index_def') THEN
        'Lifecycle notification index missing from pg_index catalog.'
      WHEN r.state = 'absent' AND r.check_kind = 'comment_col' THEN
        'Column COMMENT missing; metadata-only drift possible.'
      WHEN r.state = 'absent' AND r.check_kind = 'prereq_table' THEN
        'Prerequisite table missing; apply earlier Phase 9E/9F migrations first.'
      WHEN r.state = 'absent' THEN
        'Expected catalog object missing; compare migration file and migration history.'
      WHEN r.state = 'conflicting' AND r.check_kind = 'column_attr' THEN
        'Column type, nullability, or default differs from migration 202606200009.'
      WHEN r.state = 'conflicting' AND r.check_kind IN ('index','index_def') THEN
        'Index catalog mismatch: uniqueness, key columns, or partial predicate differs.'
      WHEN r.state = 'conflicting' AND r.check_kind = 'no_policy' THEN
        'Unexpected INSERT or adviser policies on client_notifications.'
      WHEN r.state = 'conflicting' AND r.check_kind = 'rls' THEN
        'Row level security disabled on client_notifications.'
      WHEN r.state = 'conflicting' THEN
        'Live catalog differs from migration expectation.'
      WHEN r.state = 'unknown' AND r.check_kind = 'seed' THEN
        'document_event_notifications feature row absent; seed probe could not confirm.'
