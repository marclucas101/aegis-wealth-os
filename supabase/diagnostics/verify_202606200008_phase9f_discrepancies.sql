-- Read-only discrepancy report for 202606200008_phase9f_scheduled_publishing.sql
-- Filtered view of the same resolved inventory as verify_202606200008_phase9f_scheduled_publishing.sql.

WITH
-- PHASE9F_RESOLVED_CORE_BEGIN
expected AS (
  SELECT * FROM (VALUES
...
discrepancies AS (
  SELECT
    ...
    r.state,
    r.expected_detail,
    r.detail AS actual_detail,
    r.expected_canonical_detail,
    r.actual_canonical_detail,
    CASE
      WHEN r.state = 'absent' AND r.check_kind = 'table' THEN
        'Table missing from public schema; migration 202606200008 may not be applied, was rolled back, or object was renamed.'
        || CASE WHEN r.migration_recorded IS false THEN ' schema_migrations has no 202606200008 row (migration history gap).' ELSE '' END
      WHEN r.state = 'absent' AND r.check_kind = 'column' THEN
        'Column ' || COALESCE(r.expected_detail, '?') || ' missing on ' || r.expected_object
        || '; compare migration file and live DDL.'
      WHEN r.state = 'absent' AND r.check_kind = 'constraint' THEN
        'CHECK constraint missing; migration constraint was not created or was renamed.'
      WHEN r.state = 'absent' AND r.check_kind = 'fk' THEN
        'Foreign key missing; parent/child relationship from automation_job_items to automation_job_runs not present.'
      WHEN r.state = 'absent' AND r.check_kind IN ('index','index_def') THEN
        'Index missing from pg_index catalog; migration index was not created or was renamed.'
      WHEN r.state = 'absent' AND r.check_kind = 'rls' THEN
        'Table missing or RLS state could not be resolved from pg_class.'
      WHEN r.state = 'absent' THEN
        'Expected catalog object missing; compare migration file and migration history.'
      WHEN r.state = 'conflicting' AND r.check_kind = 'constraint' THEN
        'Constraint definition differs; CHECK allowed-value lists may differ in wording, order, or qualification.'
      WHEN r.state = 'conflicting' AND r.check_kind = 'fk' THEN
        'Foreign key references unexpected table or referential action differs.'
      WHEN r.state = 'conflicting' AND r.check_kind = 'index' THEN
        'Index catalog mismatch: schema, table, uniqueness, ordered key columns, sort direction, or predicate presence differs from migration 202606200008.'
      WHEN r.state = 'conflicting' AND r.check_kind = 'index_def' THEN
        'Partial index predicate differs; compare expected_canonical_detail and actual_canonical_detail (canonicalised predicate text).'
      WHEN r.state = 'conflicting' AND r.check_kind = 'rls' THEN
        'Row level security is disabled on a table the migration enables.'
      WHEN r.state = 'conflicting' AND r.check_kind = 'no_policy' THEN
        'Unexpected client or adviser RLS policies on automation job tables; migration expects zero client/adviser policies.'
      WHEN r.state = 'conflicting' AND r.check_kind = 'comment' THEN
        'Table COMMENT ON text differs or is missing; metadata-only drift possible.'
      WHEN r.state = 'conflicting' AND r.check_kind = 'seed' THEN
        'Feature-control seed scheduled_content_automation is enabled or absent; migration expects disabled default with ON CONFLICT DO NOTHING.'
      WHEN r.state = 'unknown' AND r.check_kind = 'seed' THEN
        'platform_feature_controls absent or scheduled_content_automation row missing; seed probe could not confirm disabled state.'
