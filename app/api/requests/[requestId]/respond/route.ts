...
import { clientRespondToServiceRequest } from "@/lib/crm-v2/service/service";
import {
  parseJsonBodySafely,
  rateLimitOrThrow,
...
export async function POST(
  ...
  try {
    ...
    const result = await clientRespondToServiceRequest({
    ...
    return NextResponse.json(
      { ok: false, error: toPublicErrorMessage(err, "Failed to respond") },
