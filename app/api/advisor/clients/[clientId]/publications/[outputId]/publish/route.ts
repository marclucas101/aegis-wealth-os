...
import { publishOutput } from "@/lib/compliance/publicationWorkflow";
...
export async function POST(
  ...
  try {
    ...
    console.error("[publications/publish POST]", err);
    return NextResponse.json(
      { ok: false, error: toPublicErrorMessage(err, "Failed to publish output") },
      { status: 500 },
    );
  }
}
