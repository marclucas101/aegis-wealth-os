...
import { loadAdvisorClientShieldDiagnosticView } from "@/lib/supabase/advisorClientFinancialViews";
...
export type AdvisorClientShieldDiagnosticResponse =
  | ({ ok: true; readOnly: true } & ShieldDiagnosticSnapshot)
  | {
      ok: false;
      reason:
        | "unauthenticated"
...
): Promise<NextResponse<AdvisorClientShieldDiagnosticResponse>> {
  try {
    ...
    const result = await loadAdvisorClientShieldDiagnosticView(
      access.authUser.id,
      role,
      clientId,
    );
    ...
    const message = toPublicErrorMessage(
      err,
      "Failed to load client shield diagnostic",
    );
    console.error(
      "[api/advisor/clients/[clientId]/shield-diagnostic GET]",
      err,
    );
