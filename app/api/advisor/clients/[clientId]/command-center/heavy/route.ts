...
import {
  loadAdvisorClientCommandCenterHeavy,
  type AdvisorClientCommandCenterHeavyPayload,
} from "@/lib/supabase/advisorClientCommandCenter";

export const dynamic = "force-dynamic";

export type AdvisorClientCommandCenterHeavyResponse =
  | ({ ok: true } & AdvisorClientCommandCenterHeavyPayload)
  | {
      ok: false;
      reason:
        | "unauthenticated"
...
): Promise<NextResponse<AdvisorClientCommandCenterHeavyResponse>> {
  try {
    ...
    const rateLimit = rateLimitOrThrow<AdvisorClientCommandCenterHeavyResponse>(
      request,
      {
    ...
    const result = await loadAdvisorClientCommandCenterHeavy(
      access.authUser.id,
      role,
      clientId,
    );
    ...
    console.info(
      "[api/advisor/clients/[clientId]/command-center/heavy] timing",
    ...
    const message = toPublicErrorMessage(
      err,
      "Failed to load client workspace panels",
    );
