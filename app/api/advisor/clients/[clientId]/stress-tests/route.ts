...
import {
  loadAdvisorClientStressTestsView,
  type AdvisorClientStressTestsView,
} from "@/lib/supabase/advisorClientFinancialViews";
...
export type AdvisorClientStressTestsResponse =
  | ({ ok: true; readOnly: true } & AdvisorClientStressTestsView)
  | {
      ok: false;
      reason:
        | "unauthenticated"
...
): Promise<NextResponse<AdvisorClientStressTestsResponse>> {
  try {
    ...
    const result = await loadAdvisorClientStressTestsView(
      access.authUser.id,
      role,
      clientId,
    );
    ...
    const message = toPublicErrorMessage(
      err,
      "Failed to load client stress tests",
    );
    console.error("[api/advisor/clients/[clientId]/stress-tests GET]", err);
