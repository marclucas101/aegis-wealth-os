...
export async function GET(): Promise<NextResponse<AdvisorOnboardingListResponse>> {
  try {
    ...
    const clients = await loadOnboardingClients({
      scope: "advisor",
    ...
    console.error("[api/advisor/client-invitations GET]", err);
...
export async function POST(
  request: Request,
): Promise<NextResponse<AdvisorInviteClientResponse>> {
  try {
    const access = await requireAdvisorAccess();
    ...
    const rateLimit = rateLimitOrThrow<AdvisorInviteClientResponse>(request, {
    ...
    if (!result.ok) {
      await writeAuditLog({
        ...
        metadata: {
          email: emailResult.value,
          advisor_user_id: access.authUser.id,
    ...
    console.error("[api/advisor/client-invitations POST]", err);
