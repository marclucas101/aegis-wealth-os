...
import { requestContentChanges } from "@/lib/communications/contentWorkflow";
...
export async function POST(
  ...
  try {
    ...
    const updated = await requestContentChanges({
      contentId,
    ...
    console.error("[api/admin/communications/[contentId]/request-changes POST]", err);
    return NextResponse.json(
      { ok: false, error: toPublicErrorMessage(err, "Request changes failed") },
      { status: 500 },
    );
  }
}
