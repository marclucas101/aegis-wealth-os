...
import {
  createRelationshipMoment,
  loadAdviserMomentsWorkspace,
} from "@/lib/crm-v2/moments/moments";
import type { CreateMomentInput } from "@/lib/crm-v2/moments/types";
...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json(
      { ok: false, error: toPublicErrorMessage(err, "Failed to load moments workspace") },
...
export async function POST(
  ...
  try {
    ...
    const body = (await request.json()) as CreateMomentInput;
    const { relationshipId } = await context.params;
    const result = await createRelationshipMoment({
      authUserId: access.authUser.id,
      userRole: access.user.role as "advisor" | "admin",
      relationshipId,
      payload: body,
    ...
    return NextResponse.json(
      { ok: false, error: toPublicErrorMessage(err, "Failed to create moment") },
      { status: 500, headers: { "Cache-Control": PRIVATE_CACHE } },
    );
  }
}
