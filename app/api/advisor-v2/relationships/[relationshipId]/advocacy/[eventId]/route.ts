...
import { updateAdvocacyEvent } from "@/lib/crm-v2/advocacy/advocacy";
import type { UpdateAdvocacyEventInput } from "@/lib/crm-v2/advocacy/types";
...
export async function PATCH(
  ...
  try {
    ...
    const body = (await request.json()) as UpdateAdvocacyEventInput;
    const { relationshipId, eventId } = await context.params;
    const result = await updateAdvocacyEvent({
    ...
    return NextResponse.json(
      { ok: false, error: toPublicErrorMessage(err, "Failed to update advocacy event") },
