...
import { retryAppointmentGoogleSync } from "@/lib/crm-v2/google-calendar/service";
...
export async function POST(
  ...
  try {
    ...
    const result = await retryAppointmentGoogleSync({
    ...
    return NextResponse.json(
      { ok: false, error: toPublicErrorMessage(err, "Failed to retry Google Calendar sync") },
